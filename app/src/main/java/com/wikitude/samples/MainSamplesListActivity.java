...
public class MainSamplesListActivity extends ListActivity {
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		try {
            ...
            String activity_title_string = this.getListLabels()[position];
            intent.putExtra(EXTRAS_KEY_ACTIVITY_TITLE_STRING, activity_title_string);

            String architect_world_url = this.getArchitectWorldUrls()[position];
			intent.putExtra(EXTRAS_KEY_ACTIVITY_ARCHITECT_WORLD_URL, "samples"
					+ File.separator + architect_world_url
					+ File.separator + "index.html");

            boolean activity_ir = this.getActivitiesIr()[position];
			intent.putExtra(EXTRAS_KEY_ACTIVITY_IR, activity_ir);

            boolean activity_geo = this.getActivitiesGeo()[position];
            intent.putExtra(EXTRAS_KEY_ACTIVITY_GEO, activity_geo);
